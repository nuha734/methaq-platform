...
st.markdown(
    '<div class="subtitle">منصة ذكية للفحص الأولي للامتثال والخصوصية</div>',
    unsafe_allow_html=True
)
...
st.markdown("""
<div class="hero-box">
    <div class="hero-title">🔎 افحص متجرك وسياسة الخصوصية</div>
    <div class="hero-text">
        يساعدك ميثاق على اكتشاف المؤشرات التي قد تحتاج إلى مراجعة
        في سياسات المتجر والخصوصية، مع عرض الدليل والتوصية المناسبة.
    </div>
</div>
""", unsafe_allow_html=True)
...
def get_evidence(text, patterns):
    ...
    for sentence in sentences:
        if sentence_has_patterns(sentence, patterns):
            return sentence
        ...
        if rule.get("special") == "retention":
            ...
            if has_retention and has_duration:
                ...
                for sentence in split_sentences(text):
                    if (
                        sentence_has_patterns(sentence, patterns)
                        and sentence_has_patterns(
                            sentence,
                ...
                status = "🟡"
                evidence = get_evidence(text, patterns)
    ...
    for rule in STORE_RULES:
        ...
        results.append({
            ...
            "recommendation":
                ...
                "لم يظهر في المحتوى المتاح مؤشر واضح لهذا المتطلب، ويُفضّل مراجعته والتحقق منه."
        })
...
# =========================================================
# العثور على صفحة سياسة الخصوصية
# =========================================================

def find_privacy_page(base_url, links):
...
def display_result(result):
    ...
    if result.get("evidence"):
        ...
        with st.expander("🔎 لماذا ظهرت هذه النتيجة؟"):
...
if st.session_state.active_tab == "store":
    ...
    st.markdown(
        '<div class="section-title">🔎 فحص المتجر</div>',
        unsafe_allow_html=True
    )
